...
Version 0.3 keeps on-device inference as the default and adds **Settings → Choose your storyteller → Connect an API or local server**. Each provider has a separate saved configuration. Enter an API key and model ID, confirm the destination, and select **Use this connection**. **Load model list** contacts the configured API with your key but sends no project text. It is optional; exact model IDs can be entered manually.
